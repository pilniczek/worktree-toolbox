...
keybinding that opens one). Indirect leaks count too - keybinding chords, artifact names
(`WORK-REPORT.md`, `/work-report`), and relative paths (`../vscode-claude-tabs/`) all name a sibling
a solo install may not have. Cross-tool integration recipes belong **only** in the
...
**duplicated across all three `install.sh` on purpose** - each must run standalone from a piped
...
`test/install-matrix.sh` smoke-tests all 7 non-empty tool combinations together, asserting they
don't collide. It is fully **offline** and **sandboxed** (`HOME`, `INSTALL_DIR`, and the VS Code
...
Significant or contested choices go in `docs/adr/`. Read before re-opening a settled question.
