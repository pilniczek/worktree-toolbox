...
Each command is a **thin wrapper**: the installed `.claude/commands/<name>.md` just runs `scripts/worktree-<name>.sh` and
relays its output. All the logic — mode detection, branch-name validation, the flat-name rule, the `git worktree` calls —
lives in those scripts, with the shared rules in `scripts/worktree-common.sh`. To change behavior, edit the script: it is
