...
if [ -n "${WORKTREE_TOOLBOX_SRC:-}" ]; then  # NOSONAR: POSIX sh
  ...
  if [ -n "$SELF_DIR" ] && [ -f "$SELF_DIR/$CMD" ]; then SRC="$SELF_DIR"; fi  # NOSONAR: POSIX sh
fi
...
[ -f "$SRC/$CMD" ] || die "command not found at $SRC/$CMD."  # NOSONAR: POSIX sh
...
if [ -f .gitignore ] && grep -qxF '/WORK-REPORT.md' .gitignore; then  # NOSONAR: POSIX sh
